...
const OTP = () => {
  ...
  useEffect(() => {
    api
      .get(`/api/auth/sendOTP?phone=${params.phoneno}`)
      .then((res) => {
        if (res.data.data.verification.status === "pending")
  ...
  const otpSubmitHandler = (e) => {
    e.preventDefault();
    api
      .get(`api/auth/verifyOTP?phone=${params.phoneno}&code=${otp}`)
      .then((res) => {
        if (res.data.data.verification_check.status === "approved") {
